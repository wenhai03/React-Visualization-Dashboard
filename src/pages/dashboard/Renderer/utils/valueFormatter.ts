...
export function timeFormatter(val, type: 'seconds' | 'milliseconds' | 'microseconds', decimals) {
  ...
  _.forEach(timeMap, (item) => {
    const _val = val / item.value / (type === 'milliseconds' ? 1000 : type === 'microseconds' ? 1000_000 : 1);
    if (_val >= 1) {
      newVal = _val;
      unit = item.unit;
      return false;
