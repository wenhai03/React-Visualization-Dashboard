...
function FormCpt(props: ModalWrapProps & IProps) {
  ...
  const { datasourceValue, action, visible, initialValues = {}, destroy, range, onOk, admin, curBusiId } = props;
  ...
  useEffect(() => {
    getLabels(datasourceValue, '', range, curBusiId).then((res) => {
      setLabels(res);
        ...
        <Form
          ...
          onValuesChange={(changedValues, allValues) => {
            if (changedValues.filters) {
              ...
              getLabels(datasourceValue, `${filtersStr ? `{${filtersStr}}` : ''}`, range, curBusiId).then((res) => {
                setFilteredLabels(res);
              });
            }
          }}
            ...
            <Button
              onClick={() => {
                ...
                const requests = _.map(_labels, (item) => {
                  return getLabelValues(datasourceValue, item, range, curBusiId, filtersStr ? `{${filtersStr}}` : '');
                });
