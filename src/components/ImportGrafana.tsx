...
import { createDashboardInfo } from '@/pages/dashboardBuiltin/services';
import { convertDashboardGrafanaToN9E, JSONParse } from '@/pages/dashboard/List/utils';

type ModalType = 'dashboard' | 'dashboard-builtin';
...
function ImportGrafana(props: IProps & ModalWrapProps) {
  const { t } = useTranslation('dashboard');
  const { visible, destroy, id, type, refreshList, cateInfo } = props;
      ...
      <Form
        ...
        onFinish={(vals) => {
          ...
          if (type === 'dashboard') {
            ...
            // 内置仪表盘
            createDashboardInfo(cateInfo!.code, {
              ...data,
              tags: '',
              mode: 1,
              configs: JSON.stringify(data.configs),
...
export default ModalHOC(ImportGrafana);
