...
export default function FieldWithEditor(props: IProps) {
  const { value, onChange, extensions, renderPreview, titleExtra, record, editable = true } = props;
        ...
        {previewResult ? (
          <div className='template-field-preview'>
            {previewResult?.success ? (
              <>{renderPreview && renderPreview(previewResult?.content)}</>
            ) : (
              <div className='text-error-color'>{previewResult?.message}</div>
            )}
          </div>
        ) : (
          <CodeMirror value={value} height='100%' extensions={extensions} onChange={onChange} editable={editable} />
        )}
      </div>
    </div>
  );
