...
export default function index(props: { query: PromVisualQuery; datasourceValue: number; groupId: number }) {
  const promql = renderQuery(props.query);
  if (!promql) return null;
  return (
    <div className='prom-query-builder-rawquery-container'>
      <PromQLInput groupId={props.groupId} value={promql} datasourceValue={props.datasourceValue} readonly />
    </div>
  );
}
