...
function getTableData(options: any, busiGroup: number | undefined, query: string) {
  if (busiGroup) {
    return request(`${api.tasktpls(busiGroup)}?limit=${options.pageSize}&p=${options.current}&query=${query}`).then(
      (res) => {
...
const TaskTpl = (_props: any) => {
  ...
  const columns: ColumnProps<Tpl>[] = [
    ...
    },
    {
      ...
      dataIndex: 'title',
      render: (text, record) => {
        return <Link to={{ pathname: `/job-tpls/detail/${record.id}` }}>{text}</Link>;
      },
    },
    {
      title: t('tpl.tags'),
      ...
      render: (_text, record) => {
        return (
          <span>
            ...
            <Link to={{ pathname: `/job-tpls/modify/${record.id}` }}>{t('common:btn.modify')}</Link>
            <Divider type='vertical' />
            <Link to={{ pathname: `/job-tpls/clone/${record.id}` }}>{t('common:btn.clone')}</Link>
            <Divider type='vertical' />
            <Popconfirm
  ...
  return (
    <PageLayout title={t('tpl')} icon={<CodeOutlined />}>
      <div>
        {busiId ? (
