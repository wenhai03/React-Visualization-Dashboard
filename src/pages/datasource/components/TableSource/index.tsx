...
import { message, Table, Modal, Button, Space, Popconfirm, Input, Switch } from 'antd';
      ...
      ? [
          {
            ...
            render: (val, record) => {
              return (
                <Popconfirm
                  title={val ? t('default_type_tip1') : t('default_type_tip2', { plugin_type: record.plugin_type })}
                  showCancel={!val}
                  onConfirm={() => {
                    if (!val) {
                      updateDefaultDataSource({ id: record.id }).then((res) => {
