...
export default function Metrics(props: IProps) {
  ...
  const { datasourceValue, range, setRange, match, curBusiId } = props;
  ...
  useEffect(() => {
    if (matchStr) {
      getMetricValues(datasourceValue, matchStr, range, curBusiId).then((res) => {
  ...
  }, [refreshFlag, matchStr, curBusiId]);
