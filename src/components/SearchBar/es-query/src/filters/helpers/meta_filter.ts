 ...
 * @public
 */
export const enableFilter = (filter: Filter) => (!filter.meta.disabled ? filter : toggleFilterDisabled(filter));
 ...
 * @public
 */
export const disableFilter = (filter: Filter) => (filter.meta.disabled ? filter : toggleFilterDisabled(filter));
 ...
 * @public
 */
export const pinFilter = (filter: Filter) => (isFilterPinned(filter) ? filter : toggleFilterPinned(filter));
 ...
 * @public
 */
export const unpinFilter = (filter: Filter) => (!isFilterPinned(filter) ? filter : toggleFilterPinned(filter));
 ...
 * @public
 */
export const isFilters = (x: unknown): x is Filter[] => Array.isArray(x) && !x.find((y) => !isFilter(y));
