...
export function updatePanelsInsertRowVar(panels: IPanel[], rowId: string, varData: any) {
  const rowIndex = getRowIndex(panels, rowId);
  const newPanels: any = [...panels];
  newPanels[rowIndex] = { ...newPanels[rowIndex], var: varData };
  return newPanels;
}

export function updatePanelsInsertRowVarOptions(panels: IPanel[], rowId: string, option: any) {
  const rowIndex = getRowIndex(panels, rowId);
  const newPanels: any = [...panels];
  newPanels[rowIndex] = { ...newPanels[rowIndex], varOption: option };
  return newPanels;
}
...
export function getPanelRepeatClone(
  sourcePanel: IPanel,
  valueIndex: number,
...
export function bindRowId(panels) {
  let rowId;
  const newPanels = panels.map((element) => {
    if (element.type === 'row') {
      rowId = element.id;
      return element;
    }
...
// 合并全局和分组变量(如果分组变量有值重复优先使用分组变量)
export function mergeTwoVarOption(variableConfig, rowVarsOption) {
  const varsMap = new Map();

  rowVarsOption?.forEach((item) => {
    varsMap.set(item.name, { ...item, isRow: true });
  });

  variableConfig?.forEach((item) => {
    const key = item.name;
    const varData = varsMap.get(key);
    if (!varsMap.has(key) || !varData?.value || (isArray(varData?.value) && !varData.value.length)) {
      varsMap.set(key, item);
    }
  });

  return rowVarsOption?.length ? [...varsMap.values()] : variableConfig;
}
