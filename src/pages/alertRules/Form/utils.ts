...
export const stringifyExpressions = (
  ...
  _.forEach(expressions, (expression, index) => {
    ...
    exp += `$${expression.ref}${expression.label ? `.${expression.label}` : ''} ${expression.comparisonOperator} ${
      expression.value
    }`;
  });
  return exp;
...
export function processFormValues(values) {
  ...
  if (values.prod === 'host') {
    ...
    values.rule_config.queries = _.map(values.rule_config.queries, (item) => {
      return {
        ..._.omit(item, ['interval_unit', 'fieldsData', 'fieldsSource', 'count', 'points', 'query']),
        rule: {
          ...item.rule,
          ...(item.type === 'elastic_kql'
            ? { search_kql: buildEsQuery(item.rule.kql ?? '', item.rule.filters ?? []) }
            : {}),
        },
      };
    });
  ...
  if (values.rule_config.season) {
    values.rule_config.season = values.rule_config.season[1]
      ? [Number(values.rule_config.season[0]), Number(values.rule_config.season[1])]
      : [Number(values.rule_config.season[0])];
  }

  // let enable_in_bg = 0;
  // if (values.cate === 'prometheus') {
  //   enable_in_bg = values.enable_in_bg === false ? 0 : 1;
  // }
  ...
  const data = {
    ...
    datasource_ids: _.isArray(values.datasource_ids)
      ? values.datasource_ids
      : values.datasource_ids
      ? [values.datasource_ids]
      : [],
    relation_input: values.relation_input ? [values.relation_input] : [],
...
export function processInitialValues(values) {
  ...
  return {
    ...
    callbacks: !_.isEmpty(values?.callbacks)
      ...
      : undefined,
    effective_time:
      values?.enable_etimes && values.enable_etimes.length
    ...
    relation_input: values?.relation_input?.[0] ?? undefined,
  };
}
...
export function getDefaultValuesByProd(prod, defaultBrainParams) {
  ...
  if (prod === 'dial') {
    return {
      ...
      cate: 'prometheus',
      datasource_ids: [DATASOURCE_ALL],
      rule_config: defaultRuleConfig.dial,
    };
  }
  if (prod === 'log') {
    return {
      ...
      cate: 'elasticsearch',
      datasource_ids: [],
      rule_config: defaultRuleConfig.log,
    };
  }
  if (prod === 'apm') {
    return {
      ...
      cate: 'elasticsearch',
      datasource_ids: [],
      rule_config: defaultRuleConfig.apm,
    };
  }
  if (prod === 'logging') {
    return {
...
export function getDefaultValuesByCate(prod, cate) {
  if (cate === 'prometheus') {
    return {
      ...
      cate,
      datasource_ids: [DATASOURCE_ALL],
      rule_config: defaultRuleConfig[prod],
    };
  }
