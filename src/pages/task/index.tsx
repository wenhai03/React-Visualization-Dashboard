...
function getTableData(options: any, busiId: number | undefined, query: string, mine: boolean, days: number) {
  if (busiId) {
    return request(
      `${api.tasks(busiId)}?limit=${options.pageSize}&p=${options.current}&query=${query}&mine=${
        mine ? 1 : 0
      }&days=${days}`,
    ).then((res) => {
...
const Task = (_props: any) => {
  ...
  const { tableProps } = useAntdTable((options) => getTableData(options, curBusiId, query, mine, days), {
    refreshDeps: [curBusiId, query, mine, days],
  });
  ...
  return (
    <PageLayout title={t('task')} icon={<CodeOutlined />}>
      <div>
        {curBusiId ? (
