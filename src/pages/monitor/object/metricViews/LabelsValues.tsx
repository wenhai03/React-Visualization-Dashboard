...
export default function LabelsValues(props: IProps) {
  ...
  const { datasourceValue, value, range, curBusiId, onChange } = props;
  ...
  useEffect(() => {
    const dynamicLabelsRequests = _.map(dynamicLabels, (item) => {
      return getLabelValues(datasourceValue, item.label, range, curBusiId, filtersStr ? `{${filtersStr}}` : '');
    });
  ...
  useEffect(() => {
    ...
    const dimensionLabelsRequests = _.map(dimensionLabels, (item) => {
      return getLabelValues(datasourceValue, item.label, range, curBusiId, matchStr ? `{${matchStr}}` : '');
    });
                      ...
                      (item: string) => {
                        return (
                          <div
                            ...
                            onClick={() => {
                              const dimensionLabelValue = _.includes(dimensionLabel.value, item)
                                ? _.without(dimensionLabel.value, item)
                                : _.concat(dimensionLabel.value, item);
