...
const Panel = ({ defaultPromQL, removePanel, id, type, defaultCate }: IPanelProps) => {
  ...
  const [datasourceCate, setDatasourceCate] = useState(
    params.get('data_source_name') || localStorage.getItem(`explorer_datasource_cate_${type}`) || defaultCate,
  );
  const [datasourceValue, setDatasourceValue] = useState<string | number | null>(params.get('data_source_id'));

  useEffect(() => {
    const value = _.get(groupedDatasourceList, [datasourceCate, 0, 'id']);
        ...
        <Form.Item shouldUpdate noStyle>
          {({ getFieldValue }) => {
            ...
            if (datasourceCate === DatasourceCateEnum.elasticsearch) {
              // 获取 es 版本
              const version = groupedDatasourceList[datasourceCate]?.filter((item) => item.id === datasourceValue)[0]
                ?.settings?.version as string;
              return (
                <Elasticsearch key={datasourceValue} version={version} datasourceValue={datasourceValue} form={form} />
              );
...
const PanelList = ({ type, defaultCate }: IProps) => {
  ...
  const [panelList, setPanelList] = useState<PanelMeta[]>([
    { id: generateID(), defaultPromQL: decodeURIComponent(getUrlParamsByName('promql')) },
  ]);
  useEffect(() => {
    setPanelList([{ id: generateID(), defaultPromQL: decodeURIComponent(getUrlParamsByName('promql')) }]);
  }, [curBusiId]);
