...
const OperateForm: React.FC<Props> = ({ detail = {}, type }: any) => {
  ...
  const { busiGroups, groupedDatasourceList, curBusiGroup } = useContext(CommonStateContext);
  ...
  const onFinish = (values) => {
    ...
    if (type == 1) {
      editShield(params, detail.bgid, detail.id).then((_) => {
        message.success(t('common:success.edit'));
        history.push('/alert-mutes');
      });
    } else {
      addShield(params, detail.bgid).then((_) => {
                      ...
                      <>
                        ...
                        {fields.map(({ key, name, ...restField }) => (
                          ...
                          >
                            ...
                            {curBusiGroup.perm === 'rw' && <MinusCircleOutlined onClick={() => remove(name)} />}
                          </Space>
                        ))}
                      </>
                    )}
          ...
          {(fields, { add, remove }) => (
            <>
              <Row gutter={[10, 10]} style={{ marginBottom: '8px' }}>
                <Col span={5}>
                  <Space align='baseline'>
                    {t('tag.key.label')}
                    <Tooltip
                      title={<Trans ns='alertMutes' i18nKey='tag.key.tip' components={{ 1: <br /> }} />}
                      overlayStyle={{
                        minWidth: '330px',
                      }}
                    >
              ...
              {fields.map((field, index) => (
                <TagItem field={field} key={index} remove={remove} form={form} perm={curBusiGroup.perm} />
              ))}
            </>
          )}
        </Form.List>
