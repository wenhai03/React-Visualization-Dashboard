...
interface IProps {
  ...
  onRange?: (value?: { start: number; end: number }) => void;
}
...
export default function Timeseries(props: IProps) {
  ...
  const legendColumns =
    options.legend?.columns && options.legend?.columns.length > 0
      ? options.legend?.columns
      : ['max', 'min', 'avg', 'sum', 'last'];
  ...
  const detailFormatter = (data: any) => {
    if (detailUrl && time) {
      return getDetailUrl(detailUrl, data, dashboardMeta, time, isShare);
    }
    return;
  };
  if (!inDashboard) {
  ...
  const handleZoom = (fn) => {
    const result = fn();
    const rangArr = result[0].data;
    let value;
    if (rangArr.length) {
      value = {};
      value.start = moment.unix(rangArr[0]['0']).valueOf();
      value.end = moment.unix(rangArr[rangArr.length - 1]['0']).valueOf();
    }
    if (onRange && chartEleRef.current) {
      onRange(value);
      if (chartRef.current) {
  ...
  useEffect(() => {
    ...
    if (time) {
      ...
      if (isShare) {
        ...
        start = startAndEnd.start || Math.trunc(shareTime.start / 1000);
        end = startAndEnd.end || Math.trunc(shareTime.end / 1000);
      } else {
        const parsedRange = parseRange(time as IRawTimeRange);
        start = startAndEnd.start || moment(parsedRange.start).unix();
        end = startAndEnd.end || moment(parsedRange.end).unix();
      }
        ...
        return (
          ...
          >
            ...
            {record.offset && record.offset !== 'current' ? (
              <span style={{ paddingRight: 5 }}>offfset {record.offset}</span>
            ) : (
              ''
            )}
            <span>{_text}</span>
    ...
    >
      <div
        ...
        style={{
          height: _chartHeight,
          minHeight: '70%',
          width: placement === 'right' ? (isExpanded ? 0 : '80%') : '100%',
        }}
      />
      {hasLegend && (
        <div
          ...
          style={{
            ...
            width: placement === 'right' ? (isExpanded ? '100%' : '20%') : '100%',
            overflow: 'hidden',
            overflowY: 'auto',
          }}
        >
