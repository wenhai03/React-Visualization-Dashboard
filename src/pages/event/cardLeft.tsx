...
import './index.less';
import '@/pages/targets/BusinessGroup/style.less'
interface Props {
...
export default function CardLeft(props: Props) {
  ...
  const getList = (selectTheFirst = false) => {
    return getAggrAlerts().then((res) => {
      ...
      selectTheFirst &&
        sortedList.length > 0 &&
        !sortedList.find((item) => item.id === activeId) &&
        saveActiveId(sortedList[0].id);
      return sortedList;
    });
  };
  ...
  return (
    <div className='left-area' style={{ width: 240, background: '#fff', marginRight: 10 }}>
      ...
      {alertList
        ...
        .map((alert) => (
          <div
            className={alert.id === activeId ? 'card-menu-item is-active' : 'card-menu-item'}
            onClick={() => saveActiveId(alert.id)}
            key={alert.id}
          >
