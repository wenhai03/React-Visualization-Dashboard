...
const operateForm: React.FC<Props> = ({ type, detail = {} }) => {
  ...
  const addSubmit = () => {
    form.validateFields().then(async (values) => {
      ...
      const res = await prometheusQuery({ query: values.prom_ql }, datasourceId, curBusiId);
      if (res.error) {
        notification.error({
          message: res.error,
        });
    ...
    <div className='operate_con'>
      ...
      >
        <Space direction='vertical' style={{ width: '100%' }}>
          ...
          <Form.Item
            noStyle
            shouldUpdate={(prevValues, curValues) => prevValues.datasource_ids !== curValues.datasource_ids}
          >
          ...
          <Form.Item
            required
            label={t('prom_eval_interval')}
            tooltip={t('prom_eval_interval_tip', { num: form.getFieldValue('prom_eval_interval') })}
          >
            <Space>
              <Form.Item
