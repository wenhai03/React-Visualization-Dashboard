...
function Item(props) {
  const { item, custom, themeMode, maxValue, time, isShare } = props;
  const { baseColor = '#0065D9', displayMode = 'basic', serieWidth = 20, detailUrl } = custom as IBarGaugeStyles;
  ...
  return (
    <div className='renderer-bar-gauge-item' key={item.name}>
      <Tooltip title={item.name}>
        ...
        >
          {detailUrl ? (
            <a target='_blank' href={getDetailUrl(detailUrl, item, dashboardMeta, time, isShare)}>
              {item.name}
            </a>
          ) : (
            item.name
...
export default function BarGauge(props: IProps) {
  ...
  const curMaxValue =
    maxValue !== undefined && maxValue !== null ? maxValue : _.maxBy(calculatedValues, 'stat')?.stat || 0;
