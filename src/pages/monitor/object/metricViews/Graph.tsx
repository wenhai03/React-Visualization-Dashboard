...
export default function Graph(props: IProps) {
  ...
  const { datasourceList, curBusiId } = useContext(CommonStateContext);
  ...
  useEffect(() => {
    ...
    getLabels(datasourceValue, `${metric}${matchStr}`, range, curBusiId).then((res) => {
      setLabels(res);
    });
    ...
    <Card
      ...
      extra={
        <Space>
          ...
          <Popover
            placement='left'
            content={graphStandardOptions[chartType]}
            trigger='click'
          ...
          <a className='a-icon'>
            <ShareAltOutlined onClick={() => setVisible(true)} />
          </a>
          <a className='a-icon'>
