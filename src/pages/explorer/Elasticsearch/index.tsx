...
import { getLogsQuery, getFields } from './services';
...
export default function index(props: IProps) {
  ...
  const { datasourceValue, form, version } = props;
  ...
  }, [datasourceValue, params.get('data_source_id'), curBusiId]);
  ...
  const { run: onIndexChange } = useDebounceFn(
    (val) => {
      if (datasourceValue && val) {
        getFields(datasourceValue, val, 'date', curBusiId).then((res) => {
