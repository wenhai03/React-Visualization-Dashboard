...
import { transformColumns, transformColumnsLink } from './utils';
import './style.less';
...
const Stat = forwardRef((props: IProps, ref) => {
  ...
  const handleExportCsv = () => {
    let newColumns = resizableColumns;
    if (custom.links) {
      newColumns = resizableColumns.slice(0, resizableColumns.length - 1);
    }
    // 构建 CSV 数据
    const csvContent =
      newColumns?.map((column) => column.title).join(',') +
      '\n' +
      tableDataSource
        .map((item) =>
          resizableColumns
            .map((column) => {
              return dataReorganization(column, item);
            })
            .join(','),
        )
        .join('\n');
  ...
  // 数组重组为渲染后的结果
  const dataReorganization = (column, record) => {
    if (column.dataIndex === 'value') {
      ...
      return _.get(record.metric, column.dataIndex);
    } else if (
      displayMode === 'labelValuesToRows' &&
      targets?.map((item) => item.refId || []).includes(column.dataIndex)
    ) {
      const fieldData = record[column.dataIndex];
      let textObj = {
        text: fieldData?.text,
        color: fieldData?.color,
      };
      const overrideProps = getOverridePropertiesByName(overrides, column.dataIndex);
      if (!_.isEmpty(overrideProps)) {
        textObj = getSerieTextObj(fieldData?.stat, overrideProps?.standardOptions, overrideProps?.valueMappings);
      } else {
        textObj = getSerieTextObj(
          fieldData?.stat,
          {
