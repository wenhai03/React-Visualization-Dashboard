...
const DemoPie = (props: Props) => {
  ...
  const config: PieConfig = {
    ...
    label: {
      type: 'spider',
      content: (record) => {
        return `${labelWithName ? `${record.name}: ` : ''}${
          labelWithValue ? dataFormatter(record.value) : `${(record.percent * 100).toFixed(0)}%`
        }`;
      },
      style: {
        fontSize: 12,
    ...
    tooltip: {
      ...
      formatter: (datum) => {
        const detailDom =
          detailUrl && datum.name !== '其他'
            ? `&nbsp;|&nbsp;<span><a href=${detailFormatter(datum)} target="_blank">${detailName}</a></span>`
            : '';
